...
<body>
		...
		<div class="demoTable" style="background-color: #CAE1FF" id="gjssq">
			<div class="layui-form-item"
				...
				<div class="layui-form-item">
					...
					<button class="layui-btn" data-type="reload" type="button" id="do_search" style="margin-left: 197px;">搜索</button>
  ...
  <div class="layui-btn-container" style="width:25%;">
    <button class="layui-btn layui-btn-sm" lay-event="getCheckData" type="button">出库</button>
	...
	<script>
		layui.use([ 'table', 'form', 'layedit', 'laydate' ], function() {
			...
			table.on('toolbar(hwnrs)', function(obj) {
				var url = $('#url').val();
				var flag = $('#flag').val();
				var address="/xmssck/initXMSSCK.do";
			    		...
			    		success : function(data) {
			    			if(data.flag){
			    				layer.open({
									...
									resize : false,
									anim : 1,
									content : [ url + "xmssck/initXMSSCK.do", 'yes' ]
								});
			    			}else{
			...
			table.on('row(hwnrs)', function(obj) {
				...
				layer.open({
					...
					resize : false,
					anim : 1,
					content : [url + "xmssck/xmcgckInforShow.do?id="+id, 'yes' ]
				});
			});
